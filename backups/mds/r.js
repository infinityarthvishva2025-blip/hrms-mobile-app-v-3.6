...
const ReportsScree = ({ navigation }) => {
    const { role } = useAuth();
    const normalizedRole = role?.toLowerCase();

    // roler based access : visible only for directors , gm manager and vp
    ...
    const fetchReports = useCallback(async () => {
        ...
        setError(null);
        try {
            const response = await api.get('/DailyReportsApi/inbox', {
                params: { selectedDate }
    ...
    const markAsRead = async (reportId) => {
        ...
        try {
            ...
            // update local state : set i read to true for that report
    ...
    const renderItem = ({ item }) => (
        <View style={styles.row}>
            ...
            {/* date */}
            <Text style={[style.cell, styles.colDate]}>
                {formatDate(item.createdDate)}

            </Text>
            {/* report col with view button */}
            <View style={[styles.cell, styles.colReport]}>
                <TouchableOpacity style={styles.viewButton} onPress={() => { setSelectedReport(item); setModalVisible(true) }}>
